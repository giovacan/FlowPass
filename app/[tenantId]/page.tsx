...
export default function PublicTenantPage() {
  ...
  return <TenantPage negocio={negocio} tenantId={tenantId} />
}
...
function TenantPage({ negocio, tenantId }: { negocio: NegocioData; tenantId: string }) {
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        :root {
          --bg:       ${negocio.plantilla === 'oscuro' ? '#0F0F13' : '#F8F7F5'};
          --surface:  ${negocio.plantilla === 'oscuro' ? '#18181D' : '#FFFFFF'};
          --ink:      ${negocio.plantilla === 'oscuro' ? '#FFFFFF' : '#0F0F13'};
          --ink-dim:  ${negocio.plantilla === 'oscuro' ? 'rgba(255,255,255,0.45)' : 'rgba(15,15,19,0.5)'};
          --ink-mute: ${negocio.plantilla === 'oscuro' ? 'rgba(255,255,255,0.15)' : 'rgba(15,15,19,0.15)'};
          --line:     ${negocio.plantilla === 'oscuro' ? 'rgba(255,255,255,0.07)' : 'rgba(15,15,19,0.08)'};
          --accent:   #FF5C1F;
          --accent-2: #E04510;
        }
      ...
      {/* NAV */}
      <nav style={{ position: 'sticky', top: 0, zIndex: 50, background: negocio.plantilla === 'oscuro' ? 'rgba(15,15,19,0.85)' : 'rgba(248,247,245,0.85)', backdropFilter: 'blur(12px)', borderBottom: '1px solid var(--line)' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 24px', height: 60, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
          ...
          <a href={waLink} target="_blank" rel="noopener noreferrer"
            style={{ background: 'var(--accent)', color: '#fff', padding: '9px 18px', borderRadius: 9, fontSize: 13, fontWeight: 600, textDecoration: 'none', transition: 'filter .15s' }}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 20 }}>
          ...
          {negocio.telefono && (
            <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 16, padding: 32 }}>
              ...
              <a href={waLink} target="_blank" rel="noopener noreferrer" style={{ fontSize: 15, color: 'var(--accent)', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <footer style={{ borderTop: '1px solid var(--line)', padding: '32px 24px', textAlign: 'center' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
          ...
          <span style={{ fontSize: 11, color: 'var(--ink-mute)', fontFamily: 'monospace' }}>
            Powered by FlowPass
          </span>
        </div>
      </footer>
